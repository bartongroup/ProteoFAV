...
def get_logfile_handler(script_args):
    """ Set up a logfile handler with a standard set of formatting options.
    ...
    Here we setup the file handler and the log message formats based on the
...
def standard_logger(script_version, cmd_line, script_args, pos_args, kw_args,
    ...
    instantiated and is designed to work with a set of command line options
    that have been parsed with the `argparse package
    <http://docs.python.org/2.7/library/argparse.html>`_ passed to the routine.
